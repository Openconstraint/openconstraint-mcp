...
class RosterModel:
    ...
    def _symbol_literal(
        ...
        Every symbol reduces to a literal, which is what keeps the encoding
        small: a match window is then a plain conjunction.
        """
        ...
        if kind == "notgroup":
            # "anything outside the group", a day off included, for the same
            # reason. BCV-3.46.2 writes its free days this way.
    ...
    def _in_group_literal(self, employee_id: str, day: int, group: str) -> cp_model_helper.Literal:
        ...
        if set(members) == set(self.instance.shift_types):
            ...
            # Resolved through <ShiftGroups>, not assumed.
            ...
            for match in contract.matches:
                ...
                for pattern in match.patterns:
                    ...
                    if len(pattern.symbols) == 1:
                        ...
                        observed.extend(
                            self._symbol_literal(employee.id, start, pattern.symbols[0])
                            for start in starts
                        )
                        continue
                    for start in starts:
