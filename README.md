...
| `examples/guillotine_cutting` | packing/knapsack | checker-backed CP-SAT solve of two-dimensional guillotine cutting (fixed orientation, maximum quantities, unlimited stages) whose model decides the cut tree, plus a shelf-packing comparison graded by the same checker | `run_cpsat_python_file`, `run_cpsat_python_file_checked` | `tests/test_guillotine_cutting_checker.py`, `tests/test_guillotine_cutting_model.py`, `tests/test_guillotine_cutting_parse.py`, `tests/test_guillotine_cutting_integration.py` (integration only) | the model proves the authored `polarizing_film.json` optimal in under a second, but does not yet reproduce the published optima of the three shipped 2DPackLib instances (`CGCUT1` 244 found but unproven; `OF1`/`OF2` below 2737/2690 after 60 s), so no test asserts them |
| `examples/assembly_line_balancing` | scheduling/assembly | checker-backed CP-SAT solve of simple assembly line balancing, type 1 (fixed cycle time, minimize stations) with the Bukchin & Raviv (2018) CP model in three selectable formulations (base, station-window bounds, pairwise distances) plus a total-work bound on the station count (the default), raced with `run_cpsat_python_experiment` | `run_cpsat_python_file_checked`, `run_cpsat_python_experiment` | `tests/test_assembly_line_balancing_checker.py`, `tests/test_assembly_line_balancing_model.py`, `tests/test_assembly_line_balancing_parse.py`, `tests/test_assembly_line_balancing_integration.py` (integration only) | ships seven Scholl instances across three sizes; within 30 s the small ones prove their published optima, the medium and large ones do not all (`problem.txt`); the upper bound comes from one deterministic greedy run instead of the paper's 100 randomized ones; no formulation dominates on larger Scholl graphs (see `problem.txt`) |
