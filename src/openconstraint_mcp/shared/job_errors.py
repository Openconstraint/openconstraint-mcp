...
``JobRejectedError`` was previously in the MiniZinc job registry
(``minizinc.jobs.registry``), which imports ``minizinc.core``. Moving it here lets
``pyexec/jobs.py`` raise the same type without pulling the MiniZinc path into the
CP-SAT subtree — per AGENTS.md ("extract a shared primitive to a dependency-light
leaf both import", precedent ``runtime_install/errors.py``). ``now_ms`` and
``exception_summary`` were each duplicated identically across the MiniZinc
registries and ``pyexec/jobs.py``; they live here for the same reason.
"""
